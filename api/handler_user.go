...
func (s *Server) createDefaultStrategies(userID string, lang string) error {
	...
	locales := map[string]strategyLocale{
		"zh": {
			defaultStrategy: strategyI18n{"NOFX GLM Auto Strategy", "The only built-in strategy: read the AI direction board each cycle, load direction history and cost/liquidation heatmaps, then manage positions by the live board."},
		},
		"en": {
			defaultStrategy: strategyI18n{"NOFX GLM Auto Strategy", "The only built-in strategy: read the AI direction board each cycle, load direction history and cost/liquidation heatmaps, then manage positions by the live board."},
		},
		"id": {
			defaultStrategy: strategyI18n{"Strategi Otomatis NOFX GLM", "Satu strategi bawaan: membaca papan arah AI setiap siklus, memuat riwayat arah dan heatmap biaya/likuidasi, lalu mengelola posisi mengikuti papan langsung."},
		},
	}
	...
	setGLMStrategy := func(c *store.StrategyConfig) {
