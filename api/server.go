...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		protected := api.Group("/", s.authMiddleware())
		{
			...
			s.route(protected, "POST", "/onboarding/beginner", "Prepare beginner AI model config", s.handleBeginnerOnboarding)
			s.route(protected, "GET", "/onboarding/beginner/current", "Get current beginner AI model config", s.handleCurrentBeginnerWallet)
			...
			s.route(protected, "GET", "/vergex/direction-change/leaderboard", "Vergex OI Top 30 bull/bear direction leaderboard via AI provider", s.handleVergexDirectionChangeLeaderboard)
			s.route(protected, "GET", "/vergex/direction-change/current", "Vergex current bull/bear direction via AI provider (?symbol=BTC)", s.handleVergexDirectionChangeCurrent)
			s.route(protected, "GET", "/vergex/direction-change/history", "Vergex bull/bear direction history via AI provider (?symbol=BTC&type=all&page=1&page_size=20)", s.handleVergexDirectionChangeHistory)
			s.route(protected, "GET", "/vergex/cost-liquidation-heatmap", "Vergex cost/liquidation heatmap via AI provider (?marketType=hip3_perp&symbol=AAPL)", s.handleVergexCostLiquidationHeatmap)
			s.route(protected, "GET", "/vergex/flow-markets", "Vergex net-flow market ranking via AI provider (?chain=mainnet&window=1h&limit=25)", s.handleVergexFlowMarkets)
...
StrategyConfig fields:
  coin_source.source_type: "vergex_signal" (AI/Vergex direction board; default and recommended)
  coin_source.vergex_limit: number of AI candidates enriched with detail data (default 10, max 10)
  coin_source.vergex_market_type: "all" for the full AI board; detail calls use each ranking item's market_type
