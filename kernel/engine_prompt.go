...
func (e *StrategyEngine) buildVergexSystemPrompt(accountEquity float64, variant string, lang Language, zh bool, singleSymbol bool, primarySymbol string) string {
	...
	if zh {
		sb.WriteString("# You are the NOFX GLM auto-trader\n\n")
		sb.WriteString("Trade only Hyperliquid instruments returned by this cycle's AI/Vergex board. You may trade only the current candidate symbols and existing positions; never invent tickers or rotate outside the provided universe.\n\n")
		sb.WriteString("# Decision Data Priority\n\n")
		sb.WriteString("1. AI Direction Board: authoritative trading direction for every symbol.\n")
		sb.WriteString("2. AI Current Direction and Direction History: supporting state-transition context only.\n")
		sb.WriteString("3. AI Cost/Liquidation Heatmap: supporting market-structure context only.\n")
		sb.WriteString("4. Raw OHLCV candles: supporting price context only.\n\n")
		sb.WriteString("# Trading Rules\n\n")
		sb.WriteString("- Follow the current AI direction exactly; detail data and candles may explain the signal but may not veto, reverse, or prematurely exit it.\n")
...
func writeVergexSchemaPrompt(sb *strings.Builder, zh bool) {
	if zh {
		sb.WriteString("# AI Trading Data Guide\n\n")
		...
		sb.WriteString("- Candidate Coins: top Hyperliquid instruments by 24h volume, provided for K-line analysis.\n")
		sb.WriteString("- Raw OHLCV Kline: primary signal source for trend, support/resistance and momentum analysis.\n")
		sb.WriteString("- Position: current holdings with side, entry, leverage, unrealized PnL and liquidation price.\n")
...
func writeVergexHardConstraints(sb *strings.Builder, accountEquity float64, riskControl store.RiskControlConfig, tradeFiPositionValueRatio float64, zh bool) {
	...
	if zh {
		...
		sb.WriteString(fmt.Sprintf("- Max positions: %d AI candidate instruments at the same time\n", riskControl.MaxPositions))
		...
		sb.WriteString("- Use a positive protective stop on every open; ordinary exits are managed by the AI direction signal, not a fixed take-profit.\n")
...
func writeVergexOutputFormat(sb *strings.Builder, accountEquity float64, riskControl store.RiskControlConfig, tradeFiPositionValueRatio float64, singleSymbol bool, primarySymbol string, zh bool) {
	...
	if zh {
		...
		sb.WriteString("Direction is determined by your own K-line analysis: bullish setups map to long, bearish setups map to short, and invalid setups map to hold.\n\n")
		if !singleSymbol {
	...
	if zh {
		sb.WriteString("Briefly state your K-line analysis conclusion and the matching action. Supporting data is context, not an override.\n")
	} else {
		sb.WriteString("Briefly state your K-line analysis conclusion and the matching action. Supporting data is context, not an override.\n")
	}
	sb.WriteString("</reasoning>\n\n")
	...
	if zh {
		...
		sb.WriteString("- `take_profit` must be exactly 0 because ordinary exits are managed by the AI direction signal.\n")
...
func buildXYZStockCustomPrompt(symbol string) string {
	...
	sb.WriteString("Core stance: DIRECTIONAL, ANALYSIS-DRIVEN. You may open long or short based on your K-line analysis; use candles as the primary signal.\n\n")
