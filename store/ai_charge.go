...
// All prices set to 0 — direct AI provider, no payment gateway
...
func GetModelPrice(model string) float64 {
	// All models are free — direct API key, no payment gateway
	return 0
}
...
// IsClaw402Config checks if a trader config uses a payment provider (claw402).
// GLM and other direct-API providers do not need USDC wallet checks.
func IsClaw402Config(aiModel string) bool {
	return aiModel == "claw402"
}

// IsDirectAPIProvider returns true for providers that use direct API keys
// and do not need USDC wallet balance checks.
func IsDirectAPIProvider(aiModel string) bool {
	switch aiModel {
	case "glm", "deepseek", "openai", "claude", "gemini", "grok", "kimi", "qwen", "minimax":
		return true
	}
	return false
}
...
func EstimateRunway(usdcBalance float64, modelName string, scanIntervalMinutes int) (dailyCost float64, runwayDays float64) {
	// No cost — direct AI provider, unlimited runway
	return 0, -1
}
