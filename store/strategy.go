...
func GetDefaultStrategyConfig(lang string) StrategyConfig {
	...
	if lang == "zh" {
		config.PromptSections = PromptSectionsConfig{
			RoleDefinition: `# You are the NOFX GLM auto-trader

	Analyze the Hyperliquid instruments provided and trade based on your own K-line analysis. Raw OHLCV candles are the primary signal; use trend structure, support/resistance and momentum to decide direction.`,
			TradingFrequency: `# Trading Frequency

	- A bullish setup may open long; a bearish setup may open short.
	- Hold while your analysis confirms the existing direction.
	- Close when your analysis reverses or the setup invalidates.`,
			EntryStandards: `# Entry Standards

	Decide direction from your own K-line analysis. Supporting data may inform but cannot override your analysis.`,
			DecisionProcess: `# Decision Process

	1. Analyze the raw OHLCV candles for each candidate symbol.
	2. Hold positions where your analysis confirms the current direction; close when it reverses.
	3. For flat symbols, open long on bullish setups and short on bearish setups.
	...
	Analyze the provided Hyperliquid instruments and trade based on your own K-line analysis. Raw OHLCV candles are the primary signal; use trend structure, support/resistance and momentum to decide direction.`,
			TradingFrequency: `# Trading Frequency
...
func GetContextLimitForClient(provider, model string) int {
	if provider == "claw402" || provider == "glm" {
		switch {
