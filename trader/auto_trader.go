...
func NewAutoTrader(config AutoTraderConfig, st *store.Store, userID string) (*AutoTrader, error) {
	...
	// Payment providers (claw402) ignore customURL; GLM uses its own base URL
	switch aiModel {
	case "claw402":
	...
	case "glm":
		glmURL := customURL
		if glmURL == "" {
			glmURL = mcp.DefaultGLMBaseURL
		}
		mcpClient.SetAPIKey(apiKey, glmURL, config.CustomModelName)
	default:
	...
	// Pass wallet key to strategy engine for data routing.
	// GLM uses direct API key — no wallet needed, but pass it through
	// for providers that support data routing via API key.
	claw402Key := config.Claw402WalletKey
	if claw402Key == "" && config.AIModel == "claw402" && config.CustomAPIKey != "" {
		claw402Key = config.CustomAPIKey
	}
	// GLM: pass API key for vergex Bearer token mode (not EVM signing)
	if config.AIModel == "glm" && config.CustomAPIKey != "" {
		claw402Key = config.CustomAPIKey
	}
...
func (at *AutoTrader) reloadStrategyConfigIfChanged() error {
	...
	// GLM: pass API key for vergex Bearer token mode (not EVM signing)
	if at.config.AIModel == "glm" && at.config.CustomAPIKey != "" {
		claw402Key = at.config.CustomAPIKey
	}
...
// GLM and other direct-API providers skip this check entirely.
func (at *AutoTrader) runPreLaunchChecks() {
	if !store.IsClaw402Config(at.config.AIModel) {
		return
	}
	if store.IsDirectAPIProvider(at.config.AIModel) {
		return
	}
