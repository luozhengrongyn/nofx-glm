...
func (at *AutoTrader) enforceVergexSignalPolicy(decisions []kernel.Decision, ctx *kernel.Context) []kernel.Decision {
	...
	for _, decision := range blocked {
		at.logWarnf("🧭 Blocked %s %s: action conflicts with the current AI direction signal", decision.Symbol, decision.Action)
	}
	return filtered
}
...
func applyVergexSignalPolicy(
	...
	for _, position := range positions {
		...
		action := "hold"
		reasoning := fmt.Sprintf("AI direction remains %s; hold the existing %s position", bias, side)
		if !matches {
			switch side {
			case "long":
				action = "close_long"
			...
			if !present {
				reasoning = "Symbol is no longer present on the current AI direction board"
			} else {
				reasoning = fmt.Sprintf("AI direction changed to %s", bias)
			}
		}
		if base != "" {
