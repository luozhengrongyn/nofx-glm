...
type Client struct {
	baseURL    string
	privateKey *ecdsa.PrivateKey // x402 signing key (nil when using Bearer apiKey)
	apiKey     string            // direct Bearer token (when set, skips x402)
...
func NewClient(baseURL, privateKeyHex string, logger mcp.Logger) (*Client, error) {
	...
	// Try EVM private key (x402 signing) first
	if privateKeyHex == "" {
	...
	if privateKeyHex != "" {
		...
		// If it looks like an API key (not a valid EVM key), fall through to Bearer mode
		logger.Infof("🔧 [Vergex] Key is not an EVM private key, using Bearer token mode")
	}

	// Bearer token mode — use the key directly as Authorization header
	apiKey := strings.TrimSpace(privateKeyHex)
	if apiKey == "" {
		return nil, fmt.Errorf("vergex: no API key or wallet key provided")
	}
...
func (c *Client) doGET(ctx context.Context, path string, params url.Values) ([]byte, error) {
	...
	// Bearer token mode: direct API key, no x402 signing
	if c.apiKey != "" {
		req, err := buildReq()
		if err != nil {
			return nil, err
		...
		if resp.StatusCode != http.StatusOK {
			...
			return nil, fmt.Errorf("vergex request failed (%s): HTTP %d: %s", path, resp.StatusCode, string(body))
		}
		body, err := io.ReadAll(resp.Body)
		if err != nil {
			return nil, fmt.Errorf("vergex read body failed (%s): %w", path, err)
		}
		return body, nil
	}

	// x402 signing mode (legacy claw402 wallet)
